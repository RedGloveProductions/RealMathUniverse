...
VERSION = "v1.7J_control_schema_bridge"
...
class Bridge:
    ...
    def build(self) -> Dict[str, Any]:
        ...
        auto_fields=bool(mode.get("auto_fields_enabled", False)); auto_behavior=bool(mode.get("auto_behavior_enabled", False)); auto_camera=bool(mode.get("auto_camera_enabled", False))
        weights=mode.get("manual_field_weights") if isinstance(mode.get("manual_field_weights"),dict) else {}
        manual = {2:sf(weights.get("radial"),1.0),3:sf(weights.get("orbital"),1.0),4:sf(weights.get("vertical"),1.0),5:sf(weights.get("turbulence"),1.0),6:sf(weights.get("shell"),1.0),8:sf(mode.get("manual_scene_index"),0.0),18:sf(mode.get("manual_behavior_code"),0.0),19:0.0}
        ...
        obj={
            ...
            "auto_fields_enabled":auto_fields, "auto_behavior_enabled":auto_behavior, "auto_camera_enabled":auto_camera,
            ...
            "effective":{"scene_index":mapped["scene_index"],"behavior_code":mapped["behavior_code"],"behavior_authority_gate":mapped["behavior_authority_gate"],"field_weights":{"radial":mapped["radial"],"orbital":mapped["orbital"],"vertical":mapped["vertical"],"turbulence":mapped["turbulence"],"shell":mapped["shell"]}}
        }
        return obj
    def write_lock(self):
    ...
    def run(self):
        ...
        while self.running:
            ...
            atomic_write(self.effective,{"schema":"rmu.effective_control_state.v1_7J","version":VERSION,"updated_utc":utc(),"authority":{"field_weights":"auto" if obj["auto_fields_enabled"] else "manual","field_recipe":"auto" if obj["auto_fields_enabled"] else "manual","behavior":"auto" if obj["auto_behavior_enabled"] else "manual","camera":"auto" if obj["auto_camera_enabled"] else "manual","dataset_coupling":obj["dataset_coupling_mode"]},"effective":obj["effective"],"writer":obj["writer"]})
        server.server_close()
