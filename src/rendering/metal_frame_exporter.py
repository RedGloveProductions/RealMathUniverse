...
v1.3B:
- Renderer export now follows the single source authority: solver buffers.
- In geospatial_crab_field mode, render_sample_count defaults to all solver particles
  so the renderer sees 99,966 crab points instead of the old 25k preview sample.
"""
...
class MetalFrameExporter:
    ...
    def update(self, sim_time: float) -> None:
        ...
        if str(live_state.get("runtime_mode", "")) == "geospatial_crab_field":
            sample_count = source_count
        points = self._get_sampled_xyz(sample_count=sample_count)
        self._write_binary(points)
        self._write_metadata(points, sim_time, live_state, sample_count, source_count)
    ...
    def _read_live_control_state(self) -> dict:
        ...
        runtime = self._read_json(self.runtime_state_path)
        # Runtime state owns pause/geospatial source authority.
        for key in (
    ...
    def _write_metadata(self, points: np.ndarray, sim_time: float, live_state: dict, sample_count: int, source_count: int) -> None:
        ...
        if isinstance(live_state.get("collapse_behavior"), dict):
            collapse_cfg.update(live_state["collapse_behavior"])
        ...
        for key in (
            "trails_enabled", "trail_length", "grid_enabled", "center_marker_enabled",
            "horizon_ring_enabled", "compact_hud_enabled", "render_sample_count",
        ):
        ...
        metadata = {
            ...
            "source_particle_count": int(source_count),
            "dtype": "float32",
            "layout": "xyz_xyz_xyz",
            ...
            "particle_source_authority": "python_solver_buffers",
            "runtime_mode": live_state.get("runtime_mode", "standard"),
            "simulation_paused": bool(live_state.get("simulation_paused", False)),
            "physics_armed": bool(live_state.get("physics_armed", True)),
