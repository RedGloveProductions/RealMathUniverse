...
VERSION = "v1.6C_renderer_color_behavior_authority"
...
REPORT = ROOT / "output" / "v1_6C_renderer_color_behavior_authority_report.json"

def fail(message: str, backup: Path | None = None) -> None:
    print("V1.6C PATCH FAILED:", message)
...
def patch_render_encoder_species_color(text: str) -> tuple[str, list[str]]:
    ...
    new = '''        encoder.setVertexBuffer(buffer, offset: 0, index: 0)
        ...
        rmuV16BLoadSpeciesIdentityBuffersForParticleCount(Int(drawCount))
        if let rmuV16CRenderSpeciesIDBuffer = rmuV16BSpeciesIDBuffer {
            ...
            let fallbackIDs = Array(repeating: UInt32(0), count: max(1, Int(drawCount)))
