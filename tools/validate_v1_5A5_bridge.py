...
print("RealMathUniverse v1.5A5 port-safe bridge validator")
print("bridge exists:", BRIDGE.exists())
if BRIDGE.exists():
    text = BRIDGE.read_text(errors="replace")
    print("version marker present:", "v1.5A5_adaptive_vcv_port_safe_full_bridge" in text)
    print("single heartbeat writer present:", "def heartbeat_loop" in text)
    print("handler avoids JSON writes:", "No JSON writing here" in text)
